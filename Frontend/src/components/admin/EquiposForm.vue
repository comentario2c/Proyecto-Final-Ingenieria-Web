<template>
  <div class="p-6 bg-white rounded-lg shadow-lg max-w-lg mx-auto">
    <h2 class="text-2xl font-semibold mb-4 text-gray-800">Registrar Equipo</h2>

    <form @submit.prevent="guardar">
      <div class="mb-4">
        <label class="block text-gray-700">ID Equipo</label>
        <input v-model="form.ID_Equipo" type="text" class="w-full p-2 border rounded" required />
      </div>
...
// 🔹 Cargar las salas disponibles desde el backend
...
onMounted(() => {
  cargarSalas();
  if (props.equipo) Object.assign(form.value, props.equipo);
});
